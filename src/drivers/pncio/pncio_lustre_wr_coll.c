...
static int
LUSTRE_Calc_aggregator(PNCIO_File *fh,
                       MPI_Offset off,
#ifdef HAVE_MPI_LARGE_COUNT
                       MPI_Offset *len
#else
...
// if (myrank==0 && send_size[i]) printf("%s at %d: cb_nodes=%d send_size[%d]=%lld my_cb_nodes_index=%d\n",__func__,__LINE__, cb_nodes,i,send_size[i],fh->my_cb_nodes_index);
         ...
         */
        send_buf = (char **) NCI_Malloc(sizeof(char*) * cb_nodes);
 ...
 */
static void
heap_merge(const PNCIO_Access *others_req,
 ...
 */
static void
LUSTRE_Calc_my_req(PNCIO_File    *fh,
                   int            buf_is_contig,
...
*/
    ...
    if (buf_idx != NULL && buf_is_contig) {
        buf_idx[0] = (MPI_Offset *) NCI_Malloc(sizeof(MPI_Offset) * nelems);
        for (i = 1; i < cb_nodes; i++)
 ...
 */
static void
LUSTRE_Calc_others_req(PNCIO_File          *fh,
                       ...
                       PNCIO_Access       **others_req_ptr)
{
    ...
    *others_req_ptr = others_req;
    ...
    others_req[0].offsets  = (MPI_Offset*) NCI_Malloc(alloc_sz * npairs);
...
static void
comm_phase_alltoallw(PNCIO_File    *fh,
...
/*----< commit_comm_phase() >------------------------------------------------*/
static void
commit_comm_phase(PNCIO_File    *fh,
                  disp_len_list *send_list,  /* [cb_nodes] */
                  disp_len_list *recv_list)  /* [nprocs] */
{
...
#else
        MPI_Status *sts;
        sts = (MPI_Status*) NCI_Malloc(nreqs * sizeof(MPI_Status));
        MPI_Waitall(nreqs, reqs, sts);
        NCI_Free(sts);
#endif
    }
 ...
 */
static MPI_Offset
LUSTRE_Exch_and_write(PNCIO_File    *fh,
                      const void    *buf,
                      PNCIO_View     buf_view,
                      PNCIO_Access  *others_req,
                      PNCIO_Access  *my_req,
                      ...
                      MPI_Offset   **buf_idx)
{
     ...
     */
    off_list = (MPI_Offset*) NCI_Malloc(sizeof(MPI_Offset) * ntimes);
    end_loc = -1;
    for (m = 0; m < ntimes; m++)
...
// if (myrank == 0) printf("%s at %d: end_loc=%lld nbufs=%d recv_list=%s\n",__func__,__LINE__, end_loc,nbufs,(recv_list==NULL)?"NULL":"NOT NULL");
    ...
    if (end_loc >= 0 && nbufs > 0) {
        ...
        write_buf = (char **) NCI_Malloc(sizeof(char*) * nbufs);
        write_buf[0] = fh->io_buf;

        /* Similarly, receive buffer consists of nbufs sub-buffers */
        recv_buf = (char **) NCI_Malloc(sizeof(char*) * nbufs);
         ...
         */
        recv_count    = (MPI_Count**) NCI_Malloc(sizeof(MPI_Count*) * 3*nbufs);
        recv_count[0] = (MPI_Count*)  NCI_Malloc(sizeof(MPI_Count) * 3*nbufs * nprocs);
         ...
         */
        srt_off_len = (off_len_list*) NCI_Malloc(sizeof(off_len_list) * nbufs);
    }

    /* send_buf[] will be allocated in LUSTRE_W_Exchange_data(), when the use
     * buffer is not contiguous.
     */
    send_buf = (char **) NCI_Malloc(sizeof(char*) * nbufs);
...
double curT = MPI_Wtime();
    ...
    if (fh->hints->romio_cb_write == PNCIO_HINT_DISABLE) {
        ...
        st_end[0] = st_off;
        st_end[1] = end_off;
        st_end_all = (MPI_Offset*) NCI_Malloc(sizeof(MPI_Offset) * nprocs * 2);
    ...
    if (buf_view.count <= 1)
        buf_idx = (MPI_Offset**) NCI_Malloc(sizeof(MPI_Offset*) *
                                            fh->hints->cb_nodes);
