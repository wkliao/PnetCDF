...
// printf("%s at %d: SWITCH indep READ ------------------ off=%lld count=%ld\n",__func__,__LINE__, off, count);
     ...
     */
    Read_and_exch(fd, buf, datatype, nprocs, myrank,
                           ...
                           MPI_Count    *contig_access_count_ptr)
{
    ...
    MPI_Count k;
    MPI_Offset i_offset;
    MPI_Offset frd_size = 0, old_frd_size = 0;
    int st_index = 0;
    MPI_Offset n_filetypes, etype_in_filetype;
    MPI_Offset abs_off_in_filetype = 0;
    MPI_Offset bufsize;
    MPI_Offset sum, n_etypes_in_filetype, size_in_filetype;
     ...
     * user buffer datatype size) when the filetype is non-contiguous. Thus if
     * the filetype has been flattened in fd->flat_file, then its flattened
...
assert(0);
    ...
    if (filetype_is_contig) {
        ...
        *end_offset_ptr = offset_list[0] + len_list[0] - 1;
        ...
        offset = disp + n_filetypes * (MPI_Offset) filetype_extent + abs_off_in_filetype;
        ...
        while (buf_incr) {                                              \
            ...
            if (!flat_buf_sz) {                                         \
                ...
                user_buf_idx = flat_buf->indices[flat_buf_idx] +        \
                    (MPI_Offset)n_buftypes*(MPI_Offset)buftype_extent; \
