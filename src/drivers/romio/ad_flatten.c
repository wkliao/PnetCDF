...
#ifdef HAVE_MPI_LARGE_COUNT
        alloc_sz = sizeof(MPI_Count) * 2;
        tmp = (char*) NCI_Malloc(alloc_sz * new_count);
        memcpy(tmp, flat->indices, sizeof(MPI_Count) * flat->count);
        flat->indices = (MPI_Count*) tmp;

        tmp += sizeof(MPI_Count) * new_count;
        memcpy(tmp, flat->blocklens, sizeof(MPI_Count) * flat->count);
        flat->blocklens = (MPI_Count*) tmp;
        NCI_Free(flat->indices);
#else
        alloc_sz = sizeof(MPI_Offset) + sizeof(int);
        tmp = (char*) NCI_Malloc(alloc_sz * new_count);
        memcpy(tmp, flat->indices, sizeof(MPI_Offset) * flat->count);
        flat->indices = (MPI_Offset*) tmp;

        tmp += sizeof(MPI_Offset) * new_count;
        memcpy(tmp, flat->blocklens, sizeof(int) * flat->count);
        flat->blocklens = (int*) tmp;
        NCI_Free(flat->indices);
#endif
...
/* Now repeat with strides. */
                ...
                for (i = 1; i < top_count; i++) {
                    for (m = 0; m < num; m++) {
                        flatlist_node_grow(flat, j);
                        flat->indices[j] =
                            flat->indices[j - num] + stride * old_extent;
                        flat->blocklens[j] = flat->blocklens[j - num];
                        j++;
                    }
                }
...
/* simplest case, indexed type made up of basic or contiguous types */
                ...
                for (i = j, nonzeroth = i; i < j + top_count; i++) {
                    ...
                    if (blocklength > 0) {
                        ...
                        flat->blocklens[nonzeroth] = blocklength * old_extent;
                        nonzeroth++;
                    }
                }
                ...
                for (m = 1; m < ints[1]; m++) {
                    for (i = 0; i < num; i++) {
                        if (flat->blocklens[j - num] > 0) {
                            flatlist_node_grow(flat, nonzeroth);
                            flat->indices[nonzeroth] =
                                flat->indices[nonzeroth - num] + old_extent;
                    ...
                    for (m = 1; m < ints[1 + i]; m++) {
                        for (k = 0; k < basic_num; k++) {
                            if (flat->blocklens[j - basic_num] > 0) {
                                flatlist_node_grow(flat, nonzeroth);
                                flat->indices[nonzeroth] =
                                    flat->indices[j - basic_num] + old_extent;
...
/* simplest case, indexed type made up of basic or contiguous types */
                ...
                for (i = j; i < j + top_count; i++) {
                    ...
                    if (is_hindexed_block) {
                        ...
                        flatlist_node_grow(flat, i);
                        flat->indices[i] = st_offset + stride * old_extent;
                    }
                    flat->blocklens[i] = blocklength * old_extent;
                }
                *curr_index = i;
            } else {
/* vector of noncontiguous derived types */
...
/* Now repeat with strides. */
        ...
        case MPI_COMBINER_STRUCT_INTEGER:
            ...
            for (n = 0; n < top_count; n++) {
                PNCIO_Type_ispredef(types[n], &old_is_predef);
